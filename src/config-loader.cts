...
function loadConfigResolved(cwd: string, options: Record<string, unknown> = {}): ConfigResolution {
  ...
  try {
    ...
    const _baseConfig: Record<string, unknown> = {
      ...
      // #4238: NOT a GLOBAL_DEFAULTS_RESOLUTION_KEYS member, so it keeps its
      // project-only read. Branch D does not project it either, so routing it
      // through projectOr() here would make a global `agent_tools` honored
      // under a project config and ignored without one — the exact asymmetry
