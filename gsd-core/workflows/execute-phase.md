...
`USE_WORKTREES` and `ISOLATION` are also reset for the run when `worktree base-check` detects the orchestrator HEAD has diverged from the worktree fork base (#683 — e.g. an unmerged milestone branch). This runs for **any** isolated run, not only Claude: fork-base divergence is a property of the repository, so it degrades a GSD-created worktree exactly as a harness-created one. The auto-degrade prints a one-line warning to stderr and falls through to the sequential path so executors do not hit the exit-42 worktree-branch-check halt. Setting `worktree.baseRef:"head"` restores parallel execution where GSD itself creates the worktrees (orchestrator-managed runtimes — Codex, OpenCode, Kimi, Kimi Code), and on harness-isolated runtimes (Claude Code, Cursor) only when the setting lives in the **user/global** settings layer (`/config`; #1013): those harnesses do not read **project**-settings `baseRef` (#48, verified 5/5; upstream claude-code#44965), so a project-layer `"head"` there does not suppress the check — it compares against the real fork base and parallel execution returns once HEAD is merged/pushed so `origin/HEAD` matches it (#3659, #4090). The `worktree-branch-check` exit-42 guard inside each executor remains in place as a backstop.
