...
elif [ -z "$FILES_OVERRIDE" ] && [ -z "$PHASE_DIFF_FILES" ] && [ -n "$PHASE_SUMMARIES" ]; then
  # #4460: BOTH Tier-3 arms are gated on FILES_OVERRIDE being unset. Under
  # --files, Tier 1 has already filled REVIEW_FILES, so these arms were reached
  # anyway and the #2666 cross-check below appended the whole phase diff onto an
  # explicit user-supplied file list, contradicting line 144's "Skip SUMMARY/git
  # scoping entirely when --files is provided" and Tier 2's own --files guard.
  # The note arm carries the guard too: under --files there is no SUMMARY scope
  # to leave "unverified", so the note would assert a state the run is not in.
  #
...
elif [ -z "$FILES_OVERRIDE" ] && [ -n "$PHASE_DIFF_FILES" ]; then
