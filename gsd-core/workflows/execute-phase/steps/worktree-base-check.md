...
Cursor. A Claude Code `WorktreeCreate` hook, or a settings layer that does not parse, withholds
that trust and the check compares against `origin/HEAD` as before. Without the setting the fork
