...
| `dispatch-isolation.cjs` | Single owner of the closed dispatch-isolation vocabulary (`harness-worktree`/`orchestrator-worktree`/`none`) and its derived worktree-creating subset, with `isDispatchIsolation`/`isBaseCheckIsolationMode` guards; consumed by `gsd-tools.cjs`, `capability-validator.cjs`, `host-integration.cjs`, `worktree-base-ref.cjs` and pinned to the hook-side mirror by the parity test (#4561) |
