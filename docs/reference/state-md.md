...
| `progress.percent` | integer 0–100 | When progress data is available | Milestone progress composed **per phase slot**: each of `total_phases` phases owns `1/total_phases`; a closed phase fills its slot, an open phase fills it by its own summarized/plans fraction, and a declared phase with no plan files fills nothing (so unrealized future phases never produce a false 100%). Reads at most 99 while any phase is still open (#4210). The status-line progress bar is only rendered when this field is present — its absence suppresses the bar. |
